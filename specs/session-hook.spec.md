...
- !UCS-AL4 When the resolved primary differs from the current `sessionId` but the incoming id is already *known* to slop-mop — either it resolved through an alias (`primary_session_id !== newClaudeId`) or the resolved primary already has at least one commit (`commits.length > 0`) — switches to the resolved primary silently: sets `sessionId`, `sessionSource`, `commits`, `currentPrefix` from the response without raising `pendingNewSession`. Covers the common `/resume`-to-an-old-session case so the dialog only fires for genuinely novel ids
