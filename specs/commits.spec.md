...
- !SCM-W4 Each result carries: commit hash, claude trailer session id (the value from the matched `Slop-Mop-Session-Id` trailer — may be an alias rather than the primary), subject (first line, prefix-as-stored), full message, and author timestamp in unix seconds
...
- !SCM-AL1 `walk_session_commits` accepts a list of Claude session ids and matches the trailer against any of them; the row's `session_id` field surfaces the *trailer-as-stored* claude id (which may be an alias). The slop-mop primary is exposed once on the parent `SessionCommitsResult.primary_session_id`, not duplicated per row
